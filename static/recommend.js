// Fetch API key from Flask backend on page load
var my_api_key;
$.getJSON("/get_api_key", function(data){
  ...
  // Click handler for search button
    $('.movie-button').on('click', function(){
        ...
        if(title == ""){
            ...
            // Call load_details with the API key from .env
...
function show_details(movie_details, arr, movie_title, my_api_key, movie_id) {
  var imdb_id = movie_details.imdb_id ? movie_details.imdb_id : ""; // fallback if null
  ...
  var genre_list = genres.map(g => g.name);
  ...
  details = {
    ...
    'imdb_id': imdb_id, // always send, may be empty
    'poster': poster,
    'genres': my_genre,
    'overview': overview,
    'rating': rating,
    ...
    // POST details to Flask without reviews
  $.ajax({
    type: 'POST',
    url: "/recommend",
    data: details,
...
function get_individual_cast(movie_cast, my_api_key) {
    ...
    for (let i = 0; i < movie_cast.cast_ids.length; i++) {
        let cast_id = movie_cast.cast_ids[i];
        $.ajax({
            type: 'GET',
            ...
            success: function(cast_details) {
                cast_bdays.push(cast_details.birthday ?
                                (new Date(cast_details.birthday)).toDateString().split(' ').slice(1).join(' ')
                                : "Unknown");

                cast_bios.push(cast_details.biography ? cast_details.biography : "No bio available");

                cast_places.push(cast_details.place_of_birth ? cast_details.place_of_birth : "Unknown");
            },
            error: function() {
                cast_bdays.push("Unknown");
                cast_bios.push("No bio available");
                cast_places.push("Unknown");
            }
        });
    }

    console.log("DEBUG individual cast arrays:", {cast_bdays, cast_bios, cast_places});
